...
public class CategoryResourceTest {
    ...
    @Test
    public void eUpdateCategory() {
        Category trucksCategory = new TestCategoryObject(1010);
        trucksCategory.setName("All Trucks");
        trucksCategory.setVisible(Boolean.TRUE);
        trucksCategory.setHeader("header");
        trucksCategory.setImagePath("n/a");
        trucksCategory.setParent(new TestCategoryObject(1002));

        Response response =
                given()
                        .contentType(ContentType.JSON)
                        .pathParam("categoryId", 1010)
                        .body(trucksCategory)
                        .when()
        ...
        assertThat(category.getId()).isEqualTo(1010);
        assertThat(category.getName()).isEqualTo("All Trucks");
    }

    @Test
    public void fDeleteCategory() {
        // get list as is
        Response response =
                when()
                        .get("/admin/category")
        ...
        List<Map<String, ?>> oldCategoryList = JsonPath.from(oldJsonString).getList("");

        // delete a category
        response =
                given()
                        .pathParam("categoryId", 1013)
                        .when()
        ...
        List<Map<String, ?>> newCategoryList = JsonPath.from(newJsonString).getList("");

        assertThat(newCategoryList.size()).isEqualTo(oldCategoryList.size() - 1);
